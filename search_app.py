...
from search_app_index import get_all_gdrive_record_keys
...
@app.route('/search/<language>/<search_text>')
def search(search_text, language):
    all_gdrive_record_keys = [row['record_key'] for row in get_all_gdrive_record_keys()]
    min_year = 2005
    ccl_query = get_ccl_query(search_text, language)
    res = {'csv_records': 0,
           'xlsx_records': 0,
           'total_records': 0,
    ...
    with open('{}/records.csv'.format(search_dir), 'w') as csvfile:
        ...
        for record_num, record in enumerate(load_marc_data("ULI02", ccl_query, {})):
            ...
            if not pubyear or pubyear < min_year:
                ...
                csv_row = [record[field['name']] for field in get_marc_records_schema()['fields']
                          ] + [migdar_id, ccl_query, datetime.datetime.now(), record['json']]
    ...
    with open('{}/metadata.json'.format(search_dir), 'w') as f:
        json.dump({'search_text': search_text, 'language': language, 'ccl_query': ccl_query}, f)
    with open('./data/search_app/index.csv', 'a') as f:
        csvwriter = csv.writer(f)
        csvwriter.writerow([search_text, language, ccl_query, datetime.datetime.now(), search_id])
